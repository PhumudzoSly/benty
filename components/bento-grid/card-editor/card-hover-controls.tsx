...
import { BentoCard, HoverEffectType } from "@/types";
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={(value: HoverEffectType) =>
            updateCardStyle(card.id, { hoverEffect: value })
          }
        >
          <SelectTrigger>
            <SelectValue />
        ...
        <>
          <div className="space-y-2">
            <div className="flex justify-between">
              <Label>
                Transition Duration:{" "}
                {card.style.hoverTransitionDuration.toFixed(1)}s
              </Label>
            </div>
            <Slider
              ...
              step={0.1}
              onValueChange={([value]) =>
                updateCardStyle(card.id, { hoverTransitionDuration: value })
              }
            />
          </div>
          ...
          <div className="mt-2">
            <p className="text-xs text-muted-foreground">
              Hover over the card in the preview to see the effect. The hover
              effect will be included in the generated code.
            </p>
          </div>
        </>
      )}
