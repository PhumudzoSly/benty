...
export type HoverEffectType = "none" | "scale" | "lift" | "glow" | "border-glow" | "background-shift" | "text-shift"
...
type BentoGridContextType = {
  ...
  applyAnimationToAll: (animation: AnimationType, duration: number, delay: number) => void
  ...
  generateGlassmorphismStyle: (enabled: boolean, opacity: number) => Record<string, string>
  generateCode: () => string
}
...
export function BentoGridProvider({ children }: { children: ReactNode }) {
  ...
  const updateCardStyle = (cardId: number, style: Partial<CardStyle>) => {
    setGridConfig((prev) => ({
      ...prev,
      cards: prev.cards.map((card) => (card.id === cardId ? { ...card, style: { ...card.style, ...style } } : card)),
    }))
  }
  ...
  const updateCardSize = (cardId: number, colSpan: number, rowSpan: number) => {
    setGridConfig((prev) => ({
      ...prev,
      cards: prev.cards.map((card) => (card.id === cardId ? { ...card, colSpan, rowSpan } : card)),
    }))
  }
  ...
  const updateCardName = (cardId: number, name: string) => {
    setGridConfig((prev) => ({
      ...prev,
      cards: prev.cards.map((card) => (card.id === cardId ? { ...card, name } : card)),
    }))
  }
  ...
  const updateCardOrder = (cardId: number, newOrder: number) => {
    setGridConfig((prev) => ({
      ...prev,
      cards: prev.cards.map((card) => (card.id === cardId ? { ...card, order: newOrder } : card)),
    }))
  }
  ...
  const applyAnimationToAll = (animation: AnimationType, duration: number, delay: number) => {
    setGridConfig((prev) => ({
      ...prev,
      cards: prev.cards.map((card) => ({
        ...card,
  ...
  const generateCardHoverClass = (card: BentoCard) => {
    ...
    const transitionClass = `transition-all duration-${Math.round(hoverTransitionDuration * 300)}`
  ...
  const generateCardClass = (card: BentoCard) => {
    ...
    return [
      ...
      style.border && style.borderWidth > 1 ? `border-${style.borderWidth}` : "",
  ...
  const generateCode = () => {
    ...
    const hasCharts = gridConfig.cards.some((card) =>
      ["bar-chart", "pie-chart", "line-chart", "donut-chart"].includes(card.content.template),
    )
    ...
    sortedCards.forEach((card) => {
      ...
      if (card.style.glassmorphism) {
        styleProps.push(`backgroundColor: "rgba(255, 255, 255, ${card.style.glassmorphismOpacity})"`)
        styleProps.push(`backdropFilter: "blur(10px)"`)
      }

      // Add animation if not "none"
      if (card.style.animation !== "none") {
        styleProps.push(`animationDuration: "${card.style.animationDuration}s"`)
      ...
      if (card.style.hoverEffect !== "none") {
        styleProps.push(`transition: "all ${card.style.hoverTransitionDuration}s"`)
      }

      // Generate the style prop if we have any styles
      const styleString = styleProps.length > 0 ? `\n  style={{\n    ${styleProps.join(",\n    ")}\n  }}` : ""

      code += `  {/* ${card.name} */}\n`
      code += `  <Card className="${[colSpan, rowSpan, cardClass].filter(Boolean).join(" ")}"${styleString}>\n`
      ...
      switch (card.content.template) {
        ...
        case "image-top":
          ...
          code += `          <img src="${card.content.imageUrl || "/placeholder.svg?height=100&width=200"}" alt="Card image" className="w-full h-full object-cover" />\n`
          code += `        </div>\n`
          code += `        <div>\n`
        ...
        case "image-bottom":
          ...
          break
        case "image-side":
          code += `      <div className="flex ${card.content.imagePosition === "right" ? "flex-row-reverse" : "flex-row"} gap-4">\n`
          code += `        <div className="w-1/3 aspect-square overflow-hidden rounded-md">\n`
          code += `          <img src="${card.content.imageUrl || "/placeholder.svg?height=100&width=100"}" alt="Card image" className="w-full h-full object-cover" />\n`
          code += `        </div>\n`
        ...
        case "two-images":
          ...
          code += `            <img src="${card.content.imageUrl || "/placeholder.svg?height=100&width=100"}" alt="First image" className="w-full h-full object-cover" />\n`
          code += `          </div>\n`
          code += `          <div className="aspect-square overflow-hidden rounded-md">\n`
          code += `            <img src="${card.content.imageUrl2 || "/placeholder.svg?height=100&width=100"}" alt="Second image" className="w-full h-full object-cover" />\n`
          code += `          </div>\n`
          code += `        </div>\n`
          code += `        <p>${card.content.text}</p>\n`
          code += `      </div>\n`
        ...
        case "stat-card":
          ...
          code += `        <div className="text-3xl font-bold">${card.content.statData?.value || "0"}</div>\n`
          if (card.content.statData?.trend) {
            ...
            code += `          <span className="${trendColor} text-sm font-medium">${card.content.statData.trendValue || "0%"}</span>\n`
        ...
        case "pie-chart":
          if (gridConfig.useRechartsForExport) {
            ...
            code += `              <Pie\n`
            code += `                data={[\n`
            if (card.content.chartData?.labels && card.content.chartData.values) {
            ...
            code += `            <p>Pie Chart: ${card.content.chartData?.labels?.join(", ") || "No data"}</p>\n`
            code += `          </div>\n`
            code += `        </div>\n`
            code += `        <p className="text-center text-sm text-muted-foreground">${card.content.text}</p>\n`
            code += `      </div>\n`
        ...
        case "bar-chart":
          if (gridConfig.useRechartsForExport) {
            ...
            code += `          {/* Replace with your preferred chart library */}\n`
            code += `          <div className="bg-muted/20 rounded-md p-4 text-center">\n`
            code += `            <p>Bar Chart: ${card.content.chartData?.labels?.join(", ") || "No data"}</p>\n`
            code += `          </div>\n`
            code += `        </div>\n`
            code += `        <p className="text-center text-sm text-muted-foreground">${card.content.text}</p>\n`
            code += `      </div>\n`
        ...
        case "line-chart":
          if (gridConfig.useRechartsForExport) {
            ...
            code += `          {/* Replace with your preferred chart library */}\n`
            code += `          <div className="bg-muted/20 rounded-md p-4 text-center">\n`
            code += `            <p>Line Chart: ${card.content.chartData?.labels?.join(", ") || "No data"}</p>\n`
            code += `          </div>\n`
            code += `        </div>\n`
            code += `        <p className="text-center text-sm text-muted-foreground">${card.content.text}</p>\n`
            code += `      </div>\n`
        ...
        case "donut-chart":
          if (gridConfig.useRechartsForExport) {
            ...
            code += `          {/* Replace with your preferred chart library */}\n`
            code += `          <div className="bg-muted/20 rounded-md p-4 text-center">\n`
            code += `            <p>Donut Chart: ${card.content.chartData?.labels?.join(", ") || "No data"}</p>\n`
            code += `          </div>\n`
            code += `        </div>\n`
            code += `        <p className="text-center text-sm text-muted-foreground">${card.content.text}</p>\n`
            code += `      </div>\n`
        ...
        case "progress-card":
          ...
          const progressValue = card.content.statData?.value ? Number.parseInt(card.content.statData.value) : 50
          code += `        <div className="space-y-2">\n`
          code += `          <div className="flex justify-between text-sm font-medium">\n`
          code += `            <span>${card.content.statData?.label || "Progress"}</span>\n`
        ...
        case "cta-card":
          ...
          code += `        <Button className="w-full">${card.content.ctaText || "Get Started"}</Button>\n`
          code += `      </div>\n`
          break
        case "testimonial":
          code += `      <div className="space-y-4">\n`
          ...
          if (card.content.imageUrl) {
            ...
            code += `            <AvatarFallback>${card.content.author?.substring(0, 2) || "AB"}</AvatarFallback>\n`
            code += `          </Avatar>\n`
          }
          code += `          <div>\n`
          code += `            <p className="font-semibold">${card.content.author || "Anonymous"}</p>\n`
